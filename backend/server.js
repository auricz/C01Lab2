...
app.delete("/deleteNote/:noteId", async(req, res) => {
  ...
  try {
    ...
    jwt.verify(token, "secret-key", async (err, decoded) => {
      ...
      const collection = db.collection(COLLECTIONS.notes);
      const deleteRes = await collection.findOneAndDelete({
        username: decoded.username,
        _id: new ObjectId(noteId),
      });
      if (!deleteRes) return res.status(404).json({ error: "Unable to find note with given ID." });
...
app.patch("/editNote/:noteId", express.json(), async(req, res) => {
  ...
  try {
    ...
    jwt.verify(token, "secret-key", async (err, decoded) => {
      ...
      if (!oldData) return res.status(404).json({ error: "Unable to find note with given ID." });
